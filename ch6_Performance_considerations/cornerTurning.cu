/*
Note: Imagine matrix N being stored in column-major layout
*/
...
__global__ void cornerTurningKernel(float *M, float *N, float *P, int width) {
    ...
    for (int tile = 0; tile < (width + TILE_WIDTH - 1) / TILE_WIDTH; tile++) {
        ...
        if ((tile * TILE_WIDTH + ty) < width && col < width) {
            // Coalesced access (suppose N is stored in column major)
            N_s[tx][ty] = N[(tile * TILE_WIDTH + tx) * width + col];
        } else {
            N_s[tx][ty] = 0.0f;
        }
        __syncthreads();
...
void cornerTurning(float *M_h, float *N_h, float *P_h, int width) {
    // Allocate GPU memory
    float *M_d, *N_d, *P_d;
    ...
    cornerTurningKernel<<< numBlocks, numThreadsPerBlock >>>(M_d, N_d, P_d, width);
